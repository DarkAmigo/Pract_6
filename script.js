...
const createViewItem = (item, data) => {
    ...
    cartViewItemCountDec.addEventListener('click', () => {
        ...
        if (item.count === 0) {
            ...
            cartViewItemTotalItemPrice.textContent =
...
const getSelectedRating = () => {
    const checked = document.querySelector(
        'input[name="rating-selector"]:checked'
    );
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    savedCart.forEach(savedItem => {
        const renderItem = data.find(el => el.id === savedItem.id);
        if (!renderItem) return;

        renderItem.availableCount -= savedItem.count;
